...
class Project {
  async Create(
    ...
    try {
      ...
      loggerProject.info(
        `Roles created successfully for project ${projectOut.id}`
      );
    } catch (err) {
      ...
      loggerProject.info(
        `Project ${projectId} deleted successfully - CAUSE: Error when creating roles`
      );
  ...
  async delete(id) {
    ...
    if (!project) {
      loggerProject.error(
        "Someone tried to delete a project that doesn't exists | ID (project) passed: " +
          id
      );
      throw new Error("Project not found");
    }
  ...
  async getProject(id) {
    try {
      ...
      if (!project) {
        loggerProject.error(
          "Someone tried to get a project that doesn't exists | ID (project) passed: " +
            id
        );
        throw new Error("Project not found");
      }
